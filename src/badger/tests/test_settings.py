...
from badger.settings import init_settings, get_user_config_folder, ConfigSingleton, BadgerConfig, Setting 
...
class TestBadgerConfig:
    ...
    def test_get_user_config_folder_mac(self, monkeypatch):
        ...
        monkeypatch.setattr(os.path, 'expanduser', lambda x: '/Users/test/Library/Application Support/Badger')
    ...
    def test_init_settings(self):
        ...
        with patch('badger.settings.get_user_config_folder', return_value='/mock/config/folder'):
            with patch('badger.settings.ConfigSingleton', return_value=mock_config_singleton) as mock_config_cls:
                config_singleton = init_settings()
                mock_config_cls.assert_called_once_with('/mock/config/folder/config.yaml')
    ...
    def test_config_singleton_initialization(self):
        ...
        with patch('os.path.exists', return_value=True):
            with patch("builtins.open", mock_open(read_data=yaml.dump(self.mock_badger_config.dict(by_alias=True)))):
                ...
                assert config_singleton.config.BADGER_PLUGIN_ROOT.value == '/mock/plugin/root'
    ...
    def test_config_singleton_create_new_config_if_not_exists(self):
        with patch('os.path.exists', return_value=False):
            with patch('badger.settings.BadgerConfig', return_value=self.mock_badger_config):
                config_singleton = ConfigSingleton(self.config_file)
    ...
    def test_update_and_save_config(self):
        with patch('os.path.exists', return_value=True):
            with patch("builtins.open", mock_open(read_data=yaml.dump(self.mock_badger_config.dict(by_alias=True)))):
                ...
                assert config_singleton.config.BADGER_PLUGIN_ROOT.value == '/new/plugin/root'
    ...
    def test_reset_settings(self):
        with patch('os.path.exists', return_value=False):
            with patch('badger.settings.BadgerConfig', return_value=self.mock_badger_config):
                config_singleton = ConfigSingleton(self.config_file)
                with patch.object(config_singleton, 'update_and_save_config') as mock_update:
