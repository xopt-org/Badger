...
def test_create_subprocess_emits_signals(qtbot, process_creator):
    with patch("badger.gui.default.components.create_process.Process") as mock_process, \
         patch("badger.gui.default.components.create_process.run_routine_subprocess"):
        mock_process.return_value = MagicMock()
        
        with qtbot.waitSignal(process_creator.subprocess_prepared) as blocker_subprocess_prepared, \
        ...
        assert set(emitted_args.keys()) == {'process', 'stop_event', 'pause_event', 'data_queue', 'evaluate_queue', 'wait_event'}

        assert isinstance(emitted_args['data_queue'], mp.queues.Queue)
        assert isinstance(emitted_args['evaluate_queue'], mp.queues.Queue)
