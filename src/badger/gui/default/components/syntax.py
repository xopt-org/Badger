...
STYLES = {
    "keyword": format("#81A1C1"),
    "operator": format("#81A1C1"),
    "brace": format("white"),
    "defclass": format("#8FBCBB", "bold"),
    "string": format("#A3BE8C"),
    "string2": format("#A3BE8C"),
    "comment": format("#4C566A", "italic"),
    "self": format("white", "italic"),
    "numbers": format("#B48EAD"),
}
...
class PythonHighlighter(QtGui.QSyntaxHighlighter):
    ...
    def highlightBlock(self, text):
        ...
        for expression, nth, format in self.rules:
            ...
            index = expression_match.capturedStart(0)
            if index >= 0:
                ...
                if expression.pattern() in [
                    ...
                    innerIndex = self.tri_single[0].match(text).capturedStart(1)
                    if innerIndex == -1:
                        innerIndex = self.tri_double[0].match(text).capturedStart(1)
            ...
            while index >= 0:
                ...
                if (
                    expression_match.capturedStart(index)
                    in self.tripleQuotesWithinStrings
                ):
                    index += 1
                    continue
